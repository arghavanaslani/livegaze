/*
This utility take as input an aruco.js dictionary codelist and generate an opencv aruco compatible dictionary.
The sample use the ARUCO_MIP_36h12 dictionary.
...
const rotate = (src) => {
  return src[0].map((val, index) => src.map(row => row[row.length-1-index]));
};
const bin2matrix = (bin) => {
...
for(let i=0;i<codeList.length;i++) {
  const bin = _hex2bin(codeList[i], nBit);
  const r0 = bin2matrix(bin);
  const r1 = rotate(r0);
  const r2 = rotate(r1);
  const r3 = rotate(r2);
  ret.push([
    _bin2bytes(bin),
    _bin2bytes(matrix2bin(r1)),
    _bin2bytes(matrix2bin(r2)),
    _bin2bytes(matrix2bin(r3))
  ]);
}

console.log('static unsigned char DICT_ARUCO_MIP_36h12_BYTES[][4][5] = ' + JSON.stringify(ret).replace(/\[/g,'{').replace(/]/g,'}') + ';');
